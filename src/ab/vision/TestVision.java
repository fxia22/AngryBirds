...
public class TestVision implements Runnable {
	...
	//	System.out.println(state.toString());
		...
		List<Rectangle> clods = vision.findClods();
		List<Point> trajPoints = vision.findTrajPoints();
		List<Polygon> polygonWood = vision.findWoodPolygon();
		List<Polygon> polygonIce = vision.findIcePolygon();
		List<Polygon> polygonClod = vision.findClodsPolygon();
		//List<Polygon> polygonStones = vision.findStonesPolygon();
		Rectangle sling = vision.findSlingshot();


		// draw objects
		VisionUtils.drawBoundingBoxes(screenshot, clods, Color.WHITE,Color.DARK_GRAY);
		screenshot = VisionUtils.convert2grey(screenshot);
		...
		VisionUtils.drawBoundingBoxes(screenshot, woodBlocks, Color.WHITE,
				Color.ORANGE);
		VisionUtils.drawBoundingPolygons(screenshot,polygonWood,Color.blue);
		VisionUtils.drawBoundingPolygons(screenshot,polygonIce,Color.orange);
		VisionUtils.drawBoundingPolygons(screenshot,polygonClod,Color.yellow);
		//VisionUtils.drawBoundingPolygons(screenshot,polygonStones,Color.black);
