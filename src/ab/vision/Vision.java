...
public class Vision {
		...
		for (int n = 0; n < _nSegments; n++) {
			...
			for (int m = n + 1; m < _nSegments; m++) {
				if ((_colours[m] != 238) && (_colours[m] != 165)
						...
						&& (_colours[m] != 488) && (_colours[m] != 416)&&(_colours[m] != 208))
					continue;
				final Rectangle bounds2 = VisionUtils.dialateRectangle(
	...
	public List<Rectangle> findClods() {
		ArrayList<Rectangle> objects = new ArrayList<Rectangle>();
		...
		for (int i = 0; i < _nHeight; i++) {
			for (int j = 0; j < _nWidth; j++) {
				if ((_scene[i][j] != 136) || ignorePixel[i][j])
					continue;
				Rectangle obj = new Rectangle(j, i, 0, 0);
				LinkedList<Point> l = new LinkedList<Point>();
				List<Point> pointBag = new ArrayList<Point>();
	...
	public List<Polygon> findWoodPolygon() {
		//System.out.println("findWoodPolygon");
		ArrayList<Rectangle> objects = new ArrayList<Rectangle>();
		...
		//一开始不忽略任何点
	//	System.out.println("Ignoring none");
		for (int i = 0; i < _nHeight; i++) {
			for (int j = 0; j < _nWidth; j++) {
				if ((_scene[i][j] != 481) || ignorePixel[i][j])
					continue;
		...
		//		System.out.printf("A test%d",polyg.npoints);
				
			/*	Polygon poly2=new Polygon();
				poly2.addPoint(5, 10);
				poly2.addPoint(20, 30);
				poly2.addPoint(40, 80);
				poly2.addPoint(20, 20);
			*/
				LinkedList<Point> l = new LinkedList<Point>();
				List<Point> pointBag = new ArrayList<Point>();
				Point left = new Point(j,i);
				Point right = new Point(j,i);
				Point up = new Point(j,i);
				Point down = new Point(j,i);
				...
				while (true) {
					...
					if (p.y < _nHeight - 1)
						if ((_scene[p.y + 1][p.x] == 481
							...
							obj.add(p.x, p.y + 1);
							pointBag.add(new Point(p.x, p.y + 1));
							down.setLocation(p.x, p.y +1);
						//	polyg.addPoint(p.x, p.y+1);
						}
					if (p.x < _nWidth - 1)
						if ((_scene[p.y][p.x + 1] == 481
								|| _scene[p.y][p.x + 1] == 408 || _scene[p.y][p.x + 1] == 417)
							...
							obj.add(p.x + 1, p.y);
							pointBag.add(new Point(p.x + 1, p.y));
							right.setLocation(p.x +1, p.y);
					//		polyg.addPoint(p.x + 1, p.y);
					...
					if (((_scene[obj.x][obj.y]==481)||(_scene[obj.x][obj.y]==417)||(_scene[obj.x][obj.y]==408))&&
							((_scene[obj.x+obj.width][obj.y]==481)||(_scene[obj.x+obj.width][obj.y]==408)||(_scene[obj.x+obj.width][obj.y]==417))&&
							((_scene[obj.x][obj.y+obj.height]==481)||(_scene[obj.x][obj.y+obj.height]==417)||(_scene[obj.x][obj.y+obj.height]==408))&&
							((_scene[obj.x+obj.width][obj.y+obj.height]==481)||(_scene[obj.x+obj.width][obj.y+obj.height]==417)||(_scene[obj.x+obj.width][obj.y+obj.height]==408)))
						
					(true)
							{
								System.out.println("Rectangle is Polygon");
								polyg.addPoint(obj.x, obj.y);
								polyg.addPoint(obj.x, obj.y+obj.height);
								polyg.addPoint(obj.x+obj.width, obj.y);
								polyg.addPoint(obj.x+obj.width, obj.y+obj.height);
								System.out.println("Rectangle is Polygon");
							}
							
					else
					{
						System.out.println("Rectangle is not Polygon");
						for (i=obj.x;i<=obj.x+obj.width;i++)
						{
							if (_scene[i][obj.y] == 481) 
								{
								polyg.addPoint(i, obj.y);
								
								System.out.println("Point added");
								System.out.printf("%d",polyg.npoints);
								}
					...
					for (i=obj.x;i<=obj.x+obj.width;i++)
						{
							if (_scene[i][obj.y+obj.height] == 481) 
								{
								polyg.addPoint(i, obj.y+obj.height);
								System.out.println("Point added");
								System.out.printf("%d",polyg.npoints);
					...
					for (j=obj.y;j<=obj.y+obj.height;j++)
						{
							if (_scene[obj.x][j] == 481) {
								polyg.addPoint(obj.x,j);
								System.out.println("Point added");
								System.out.printf("%d",polyg.npoints);
					...
					for (j=obj.y;j<=obj.y+obj.height;j++)
						{
							if (_scene[obj.x+obj.width][j] == 481) {
								
								polyg.addPoint(obj.x+obj.width,j);
								System.out.println("Point added");
								System.out.printf("%d",polyg.npoints);
				...
				int[] ax=new int[pointBag.size()];
				int[] ay=new int[pointBag.size()];
				for (int k=0;k<pointBag.size();k++)
				{
					ax[k]=pointBag.get(k).x;
					ay[k]=pointBag.get(k).y;
					
				}
				
				ArrayList<Point> hullP = new ArrayList<Point>();
				JarvisMarch jar = new JarvisMarch(ax,ay);
				jar.calculateHull();
				hullP=jar.showHullPoints();
			//	System.out.printf("Size%d\n",hullP.size());
				for (int k=0;k<hullP.size();k++)
				{
					polyg.addPoint(hullP.get(k).x, hullP.get(k).y);
				}
			...
			//	System.out.printf("%d  %d  %d  %d\n",obj.x,obj.y,obj.height,obj.width);
				...
				if (obj.width * obj.height > _regionThreshold
					...
					{
						objects.add(obj);
						polygonobjects.add(polyg);
						//System.out.printf("Polygonsize%d\n",polyg.npoints);
					}
			}
		}
	...
	public List<Polygon> findStonesPolygon() {
		//System.out.println("findWoodPolygon");
		ArrayList<Rectangle> objects = new ArrayList<Rectangle>();
		ArrayList<Polygon> polygonobjects = new ArrayList<Polygon>();
		// test for wood (mainly 481)
		...
		//		System.out.printf("A test%d",polyg.npoints);
			...
			*/
				...
				while (true) {
					...
					if (p.y < _nHeight - 1)
						if ((_scene[p.y + 1][p.x] == 365
								|| _scene[p.y + 1][p.x] == 292 || _scene[p.y + 1][p.x] == 438)
								&& !ignorePixel[p.y + 1][p.x]) {
							l.add(new Point(p.x, p.y + 1));
							obj.add(p.x, p.y + 1);
							pointBag.add(new Point(p.x, p.y + 1));
					...
					if (p.x < _nWidth - 1)
						if ((_scene[p.y][p.x + 1] == 365
								|| _scene[p.y][p.x + 1] == 292 || _scene[p.y][p.x + 1] == 438)
								&& !ignorePixel[p.y][p.x + 1]) {
							l.add(new Point(p.x + 1, p.y));
							obj.add(p.x + 1, p.y);
							pointBag.add(new Point(p.x + 1, p.y));
	...
	public List<Polygon> findIcePolygon() {
		//System.out.println("findWoodPolygon");
		ArrayList<Rectangle> objects = new ArrayList<Rectangle>();
		ArrayList<Polygon> polygonobjects = new ArrayList<Polygon>();
		// test for wood (mainly 481)
	...
	public List<Polygon> findClodsPolygon() {
		//System.out.println("findWoodPolygon");
		ArrayList<Rectangle> objects = new ArrayList<Rectangle>();
		ArrayList<Polygon> polygonobjects = new ArrayList<Polygon>();
		// test for wood (mainly 481)
